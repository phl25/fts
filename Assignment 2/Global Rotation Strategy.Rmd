...
The GRS is based on a ranking approach. The used ETFs are ranked based on momentum which is measured by the Rate of Change (ROC) indicator. The lookback period is variable and can be tested for different values. The same is true for the invested period. For this report the lookback period will be tested for different values between one month and 12 months. The best result will be presented in the report. The time period for being invested is one month. The strategy suggests to buy the best ranked ETF. Many variations and extensions of the strategy exist which will be used and presented throughout the report. 

## Choice of ETFs

In order to achieve substantial returns it is benefitial for a ranking strategy that the selected ETFs are not too volatile and have long-term visible trends. It can be assumed that the best results can be achieved if the used ETFs have a comparable volatility. A large market volume makes sure that the ETFs cannot be easily manipulated.
This is the case for all proposed ETFs in the strategy. The 5 chosen ETFs cover the world market. The following ETFs are used: 
...
According to the strategy, the ETFs follow slightly different economic cycles. There are certain periods where one market outperforms another until it becomes overpriced. As a result investors begin to shift there money in other cheaper valued markets. With the GRS strategy, this trends can be followed.

# Implementation of the trading strategy and Benchmark
...
osInvestAll <- function (data, timestamp, orderqty, 
                         ordertype, orderside, portfolio, 
...
### Step 6: Transaction cost function
In order to include realistic transaction costs, the following transaction costs function is used.
Depending on the order volume we have increeasing transactioncosts. Ordervolume above 28000 

```{r}
# transaction fees function
txnc <- function(TxnQty, TxnPrice, Symbol, ...)
{
  ordervalue = abs(TxnQty) * TxnPrice
...
Check the orderbook to verify that all transactions are made correctly. The orderbook can be found in the [appendix](#Appendix). 
...
# present performance metrics for the entire strategy
somestats <- data.frame(rownames(tab.perf),
                        tab.perf[,1], 
                        rownames(tab.risk),
                        tab.risk[,1])
colnames(somestats) <- c("Performance Metric", 
                         "Performance Value", 
                         "Risk Metric", 
                         "Risk Value")
somestats
...
funddata$monthly.returns <- funddata$monthly.returns*100
funddata$excessrtn <- funddata$monthly.returns - funddata$rf
```
...
The results of the strategy's risk analysis indicate that no excess returns can be generated. The intercept of the regression models (CAPM as well as Fama-French model) is slightly positive but not significant. This means that risk adjusted and compared to the market we cannot generate an alpha (excess returns). However, the beta-coefficient for the market risk free return is significant and with about 0.6 lower than 1. Thus, the strategy is slightly tilted towards less risky instruments.
...
```{r message=FALSE, results="hide"}
  ...
  LoadCourseFile(BuyHoldDirectory,
                 BuyHoldInstrument,
                 debugme=TRUE,
                 dates="2003-08-31::2018")
...
```{r message=FALSE, results="hide"}
plot.zoo(equity.bh.v1,
         plot.type="single",
         col=c('red','blue'),
         main="Final Equity Comparison",
         xlab="Date",
         ylab="USD $")
legend(x="topleft",
       legend=colnames(equity.bh.v1),
       col=c("red","blue"),
       lty=1)
```
...
We can see that the Buy and Hold strategy generates higher returns while having a smaller annualized risk. The sharpe ratio is higher and therefore we cannot outperform the buy and hold strategy.
```{r message=FALSE}
chart.RiskReturnScatter(rets.bh.v1, xlim = c(0,0.3))
table.AnnualizedReturns(rets.bh.v1)
table.AnnualizedReturns(rets.v1)
...
## Conclusion
The Global Rotation Strategy in its plain generates 5.96% annulaized return while the compared buy and hold strategy generates 6.7% annualized return. The Sharp Ratio for the strategy is 0.40 compared to 0.51 for the buy and hold strategy. As we saw, the risk analysis has shown that we cannot generate excess returns with this strategy compared to the market. Especially after 2009 the strategy does not generate enough returns to outperform the buy and hols strategy. In fact, after 2009 the returns of the strategy are not significantly positive while the buy and hold strategy generates positive returns from 2009 until the end of the observation period. The following chapters describe improvement options for the strategy, in order to increase the returns and the performance, especially for the time horizon after the GFC. 
Note: Without transaction costs, the strategy in its plain form, outperforms the market in regards to annualized returns. This leads to the assumption that the holding period of the ETFs is too short in order to generate enough returns to compensate for the transaction costs.
...
initAcct(accountname.alt,
         portfolios=portfolioname.alt,
         initDate=initdate,
         initEq=startingcapital,
         currency="USD")
...
Phases where the original strategy generates negative returns, e.g., 2008 - 2009 are improved in the V2 strategy by simply not being invested in the market. This makes sense, as we only enter a trade, when the past 3 months generated positive returns, we avoid phases in which the entire world market (2008-2009) is in a negative trend. As a result, we can see that we are not invested during this time at all. 
```{r message=FALSE, results="hide"}
par(mfrow=c(2,1))
...
# Calculate Excess Returns above the risk free rate
funddata.alt$Return.V2 <- funddata.alt$Return.V2*100
funddata.alt$excessrtn <- funddata.alt$Return.V2 - funddata.alt$rf
```
...
The improved strategy's risk analysis shows that we can generate excess returns after adjusting for risk. The intercept of the regression is in both tested models (CAPM and Fama-French Model) significant and positive with about 0.55 % which leads to an excess return of 6.6 % per year. 
Moreover, the beta is also significant with about 0.37 which means that our strategy is significantly tilted towards instrument that have lower risk.

## Conclusion
We see that the improved strategy generates an annualized return of 10.9%. The annualized Sharp Ratio is 1. There are two interesting observations which I would like to summarize here. First, by using the rule that we only buy the top ranked ETF if and only if it had positive returns over the observation period, we eliminate the large drawdown during the GFC. The equity curve is just flat at that point which means that we are not invested at all. An improvement to this could be to invest in such market phases into risk free instruments as Bonds. We can minimize the maximum drawdown from 33.7 % in the original strategy to 11 % in the improved version. Second, we observe a much better performance in general after the GFC. The following chapter describes another suggestion how the strategy can be improved even further.

# Strategy Improvement V3
Now, that we eliminated the negative impact of down-phases in the market, the next improvement is to see whether we can generate profits during these phases by adding short-rules. A short position is opened for the ETF with the worse performance accoring to the momentum (ROC). Moreover, it is only opened if the return over the momentum period was negativ. The exit of the short position is triggered when ETF is not ranked at the last position anymore OR when the return over the observed period is not negative anymore.
...
## Analysis of the improved (V3) Strategy
...
As we can see, the curves look very similar, besides the fact that the scale is different for the GRS.V3 strategy. Through the implementation of short selling rules we are able to increase the returns by a significant amount. Moreover, we are able to generate returns during phases where the V2 strategy simply did not perform any trades. By this we generate returns during strong and weak market phases.
...
plot.zoo(equity.bh.v1.v2.v3,plot.type="single",
         col=c('red','blue','green', 'black'),
         main="Final Equity Comparison",
         xlab="Date",
         ylab="USD $")
legend(x="topleft",
       legend=colnames(equity.bh.v1.v2.v3),
       col=c("red","blue","green", "black"),
       lty=1)
```

### Visualizing Risk and Return
The visualization and comparison of the Risk and Return scatter shows that the risk increased minimally but the return increased significantly, compared to the GRS.V2 strategy. The annualized Sharpe Ratio is now 1.3. The annualized return is 14.9 % with an annualized standard deviation  of 11%. Moreover, the maximum drawdown decreased again compared to the previous strategy to only 7.5 %. 
```{r message=FALSE, results="hide"}
...
#### General Strategy Statistics of the improved V3 strategy
```{r}
...
### Compare returns of all strategies
...
The relative performance of the V3 strategy compared to the V2 strategy shows that especially during the financial crisis the strategy which included short-selling outperformed the GRS.V2 strategy. We can also see that before the GFC the improved strategy did not lead to the expected increase in returns. The previous strategy (V2) performed slightly better. This is due to the fact that the market was in an up-trend where shortselling was sometimes responsible for a loss in returns. However, the GFC boosted the returns of the improved V3 strategy compared to the previous V2 strategy. The main reason is the implementation of the shortselling rules.
...
for (symbol in symbollist)
{
    if (nrow(getTxns(portfolioname.alt.v3,Symbol=symbol)) > 1)
    {
        ...
        chart.ME(portfolioname.alt.v3, Symbol=symbol, type="MFE", scale="percent")
        chart.ME(portfolioname.alt.v3, Symbol=symbol, type="MAE", scale="percent")
    }
}
```
...
# Calculate Excess Returns above the risk free rate
funddata.alt.v3$Return.V3 <- funddata.alt.v3$Return.V3*100
funddata.alt.v3$excessrtn <- funddata.alt.v3$Return.V3 - funddata.alt.v3$rf
```
...
The analysis of the improved V3 strategy's risk shows again what we have seen with the V2 improvement strategy. We can generate a significant alpha of about 13.9 % per year after adjusting for risk. This can be seen as a very strong outperformance of the market.
...
rets.all <- cbind(rets.bh.v1.v2.v3[,2], rets.bh.v1.v2.v3[,4], rets.bh.v1.v2.v3[,3], rets.bh.v1.v2.v3[,1])

tab.perf <- table.Arbitrary(rets.all, metrics=c("Return.cumulative",
                                            "Return.annualized",
                                            "SharpeRatio.annualized",
                                            "CalmarRatio"),
                            metricsNames=c("Cumulative Return", 
                                           ...
                                           "Annualized Sharp Ratio",
                                           "Calmar Ratio"))
tab.risk <- table.Arbitrary(rets.all, metrics=c("StdDev.annualized",
                                            "maxDrawdown",
                                            "VaR",
                                            "ES"),
                            metricsNames=c("Annualized StdDev", 
...
The GRS is based on ranking. The 5 ETFs, which represent the world economy (US, EU, Latin America, Emergin Markets, Asia Pacific) are ranked based on momentum, specifically based on the Rate of Change (ROC) indicator. The ROC is calculated based on the monthly returns of the last 3 months. The best ranked ETF is bought and held for 1 month. If an ETF is ranked the best in two consecutive months it is simply held and not sold. Otherwise it is sold and the ETF which is now ranked best is bought. 
We clearly see that This strategy outperforms the market before the GFC but has a huge drawdown during the GFC as during this time even the best ranked ETFs generated negative returns. 
A possible improvement, with the goal to limit the losses, was to only trade, when the ROC was positive. By this, it could be ensured, that there was no investment during the GFC (see green curve above). However, this could also be further improved by investing in risk free instruments during phases when no ETF generates positive returns. 
The third step was to even further improve the strategy by not only not trading during phases of only negative returns, but also include shortselling. The result is the red equity curve above. It can be clearly shown that through shortselling of the last ranked ETF increased returns can be generated. Not only during the GFC but also during the entire time after the GFC.
The comparison of the performance and risk metrics (see table above) shows the values for all strategies.
Naive money management was used for backtesting the strategy. All available equity was used to buy the positions. Moreover, realistic transaction costs were included in the backtesting. 
In summary, the strategy itself performs quiet well. When adding transaction costs, the plain strategy does not outperform the buy and hold strategy in regards to returns and risk. The proposed improvements led to a significant increase in performance with a annualized return of 14.9%, an annualized Sharp Ration of 1.3, and a maximum drawdown of 7.5%.
The improved strategy performs excellent during different market phases. We see a steep performance during uptrends (After dot-com bubble until the GFC) but also during the GFC through shortselling and during phases where many markets performed sidewards without a clear trend. It can be assumed that such a strategy could perform as good as during the tested period in the future. 
Although an extensive test of the strategy was performed, a formal optimization approach could bring up even more profitable and robust strategies. This could be part of future research regarding the Global Rotation Strategy.
