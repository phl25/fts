...
knitr::opts_chunk$set(tidy.opts=list(width.cutoff=48),tidy=TRUE)
```
...
for (instrument in instrumentlist){
  ...
  for (daterange_check_i in daterange_check){
    print(chart.Posn(portfolioname,Symbol=instrument,type='candlesticks', theme=myTheme,subset=daterange_check_i,TA=addEMAString))
  }
}
```
...
for (instrument in instrumentlist){
  ...
  print(chart_Series(x=chart,name=instrument,theme=myTheme, subset=daterange,TA="add_TA((chart$EMA), on=1,type='l',col='blue');add_Vo()"))
}
```
...
for (instrument in instrumentlist){
  print(instrument)
  print(getTxns(Portfolio=portfolioname,Symbol=instrument))
}
```
...
for (i in 1:nrow(tstats)) {
  trades.tab <- cbind(
    ...
    c(tstats[i,"Num.Trades"],tstats[i,"Percent.Positive"],tstats[i,"Percent.Negative"],tstats[i,"Percent.Positive"]/tstats[i,"Percent.Negative"]))
...
for (i in 1:length(instrumentlist)){
  ...
  print(charts.PerformanceSummary(rets[,i],colorset=rainbow12equal,main=instrumentlist[i]))
}
```
...
```{r message=FALSE, results="hide"}
for (i in 1:length(instrumentlist)){
  print(plot(rets[,i], lwd = 1, main = paste("Returns of ", instrumentlist[i], sep="")))
  equityCurve <- cumprod(1 + rets[,i])
  print(plot(equityCurve, lwd = 1, main = paste("Equity curve of ", instrumentlist[i], sep="")))
  print(hist(rets[,i],main="Histogram of Simple Returns",breaks=50))
}
```
...
for (i in 1:length(instrumentlist)) {
  ...
  print(sprintf("A %s Euro investment in %s could lose with a probability of", Investment, instrumentlist[i], probs))
  print(round(loss,2))
}
```
...
equitySimple <- cumprod(1 + rets)
par(mfrow = c(1,1))
plot(equitySimple, lwd = 1)

equity.bh <- cumprod(1 + rets.bh)
plot(equity.bh, lwd = 1)
rm(equity.bh)
